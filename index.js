...
client.on(Events.MessageCreate, async (message) => {
	...
	if (checkCluelessKing(message.author.id.toString())) {
      ...
      const random = Math.random();
        ...
        const moreRandomStuffCauseWhyNot = Math.random();
				console.debug('[cluelessKing] not clueless check 2', moreRandomStuffCauseWhyNot);
	...
	if (checkCopiumKing(message.author.id.toString())) {
      ...
      const random = Math.random();
			...
			if (random < 0.25) {
        const moreRandomStuffCauseWhyNot = Math.random();
				console.debug('[copiumKing] not copium check 2', moreRandomStuffCauseWhyNot);
				if (moreRandomStuffCauseWhyNot < 0.5) {
					await message.reply({
			...
			if (random < 0.25) {
			  await message.reply({
				  content: '<:lookDownPestie:1512044054471245854>',
					files: [
					  {
							attachment: 'https://cdn.pestoverse.world/yunya/warlord.png',

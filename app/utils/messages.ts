...
export const PPCHECK_MESSAGES = {
    ...
    "100": ["<:yuniiuh:1298954249908125747>", "MAXIMUM OVERDRIVE ACHIEVED!"],
    Infinity: [
        "Infinity? More like... divinity. You're transcending dimensions.",
        "This broke the PPCheck scale. Please reboot the yuniiverse.",
    ],
};

export function getPPCheckMessage(power: number): string {
    if (power === Infinity)
        return PPCHECK_MESSAGES["Infinity"][
            Math.floor(Math.random() * PPCHECK_MESSAGES["Infinity"].length)
        ];
    else if (power <= -100)
...
export function getCluelessKingMessage(userId: string) {
    return checkCluelessKing(userId)

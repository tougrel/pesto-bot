    ...
    async run(client, interaction) {
        ...
        try {
            ...
            await interaction.editReply({
                content: `${user ? (member.nickname ?? user.username) : interaction.user}'s mango power is **${power_to_show}%**, ${getMangoMessage(power_to_show)} ${data !== undefined && !has_expired ? `(**Reroll** <a:pestoScam:1323758768336404500>! First feetcheck of the date was **${data.check_value}**%)` : ""}\n-# Checks reset <t:${expire_timestamp_in_seconds}:R> (<t:${expire_timestamp_in_seconds}>)`,
            });

            if (is_april_fools) {
                setTimeout(async () => {
                    await interaction.editReply({
                        content: `${user ? (member.nickname ?? user.username) : interaction.user}'s mango power is **${power}%**, ${getMangoMessage(power)} ${data !== undefined && !has_expired ? `(**Reroll** <a:pestoScam:1323758768336404500>! First feetcheck of the date was **${data.check_value}**%)` : ""}\n-# Checks reset <t:${expire_timestamp_in_seconds}:R> (<t:${expire_timestamp_in_seconds}>)`,
                    });
                }, 60 * 1000);
            }
